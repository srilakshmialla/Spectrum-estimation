...
%% Repeating periodogram for 50 realizations
%
...
%
% Any time series can be expressed as a combination of cosine (or sine) waves
% with differing periods (how long it takes to complete a full cycle) and 
% amplitudes (maximum/minimum value during the cycle).  This fact can be utilized 
% to examine the periodic (cyclical) behavior in a time series.
% 
% A periodogram is used to identify the dominant periods (or frequencies) of 
% a time series.  This can be a helpful tool for identifying the dominant 
% cyclical behavior in a series, particularly when the cycles are not related 
% to the commonly encountered monthly or quarterly seasonality.(Source:Internet)
...
%% Welch method with no segment overlaps and the hamming window
...
%% Welch method with 50% overlap between segments
...
%% Comparison of the results obtained from above methods
%
% Depending on variability of the estimate and figure of merit(in turn resolution)
% we can compare these methods.
%
% Theoretically,Blackman-Tukey method seems to be better than Welch method.
%
% Out of all the methods, Minimum Variance gives best estimate.This method 
% is adaptive and uses eigenvalue decomposition making calculations easier.
%
% Regular Periodogram seems to be having more Variance and non-accurate estimate of all.
%% Note:
%
...
%% Reference
% http://www.mathworks.com/help/signal/ug/nonparametric-methods.html
